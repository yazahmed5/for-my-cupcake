...
// ── ACTIVE NAV ───────────────────────────────────────
const sections = [
  { id: 'need-me',  link: document.querySelector('a[href="#need-me"]')  },
  { id: 'hangouts', link: document.querySelector('a[href="#hangouts"]') },
  { id: 'marathon', link: document.querySelector('a[href="#marathon"]') },
...
// Video cards use the video surface itself as the play/pause control.
...
const sunflowerBtn = document.getElementById('sunflowerSurpriseBtn');
const sunflowerModal = document.getElementById('sunflowerModal');
const sunflowerClose = document.getElementById('sunflowerClose');

function openSunflowerModal() {
  if (!sunflowerModal) return;
  sunflowerModal.classList.remove('open');
  void sunflowerModal.offsetWidth;
  sunflowerModal.classList.add('open');
  sunflowerModal.setAttribute('aria-hidden', 'false');
}

function closeSunflowerModal() {
  if (!sunflowerModal) return;
  sunflowerModal.classList.remove('open');
  sunflowerModal.setAttribute('aria-hidden', 'true');
}

sunflowerBtn?.addEventListener('click', openSunflowerModal);
sunflowerClose?.addEventListener('click', closeSunflowerModal);
sunflowerModal?.addEventListener('click', e => {
  if (e.target === sunflowerModal) closeSunflowerModal();
});
document.addEventListener('keydown', e => {
  if (e.key === 'Escape') closeSunflowerModal();
});
